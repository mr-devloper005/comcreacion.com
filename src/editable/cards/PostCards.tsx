  ...
  return (
    <Link href={href} className={`group block min-w-0 overflow-hidden ${dc.surface.dark} ${dc.motion.lift}`}>
      <div className="relative min-h-[520px] p-6 sm:p-8 lg:min-h-[620px]">
        <img src={getEditablePostImage(post)} alt={post.title} className="absolute inset-0 h-full w-full object-cover opacity-45 transition duration-500 group-hover:scale-105" />
        <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(3,4,5,0.15),rgba(3,4,5,0.92))]" />
        <div className="relative z-10 flex h-full min-h-[460px] flex-col justify-end lg:min-h-[560px]">
          ...
          <h3 className="mt-5 max-w-3xl text-4xl font-bold leading-[1.02] tracking-[-0.03em] sm:text-5xl lg:text-6xl">{post.title}</h3>
          <p className="mt-5 max-w-2xl text-sm leading-8 text-white/70 sm:text-base">{getEditableExcerpt(post, 190)}</p>
          <span className={`mt-8 inline-flex w-fit items-center gap-2 rounded-full ${pal.accentBg} px-5 py-3 text-sm font-semibold ${pal.onAccentText}`}>
  ...
  return (
    <Link href={href} className={`group ${dc.layout.minRailCard} block overflow-hidden ${dc.surface.card} ${dc.motion.lift}`}>
      <div className={`${dc.media.frame} ${dc.media.ratio}`}>
        ...
        <span className={`absolute left-4 top-4 rounded-full ${pal.darkBg} px-3 py-1 text-[10px] font-bold uppercase tracking-[0.2em] ${pal.darkText}`}>No. {String(index + 1).padStart(2, '0')}</span>
      </div>
      <div className="p-5">
        <p className={`${dc.type.eyebrow}`}>{getEditableCategory(post)}</p>
        <h3 className={`mt-3 line-clamp-3 text-2xl font-semibold leading-tight tracking-[-0.02em] ${pal.panelText}`}>{post.title}</h3>
  ...
  return (
    <Link href={href} className={`group block min-w-0 ${dc.surface.soft} p-5 ${dc.motion.lift}`}>
      <div className="flex items-start gap-4">
        <span className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-full ${pal.accentBg} text-xs font-bold ${pal.onAccentText}`}>{index + 1}</span>
        <div className="min-w-0">
          ...
          <h3 className={`mt-2 line-clamp-2 text-xl font-semibold leading-tight tracking-[-0.02em] ${pal.panelText}`}>{post.title}</h3>
  ...
  return (
    <Link href={href} className={`group grid min-w-0 gap-5 overflow-hidden ${dc.surface.card} p-4 ${dc.motion.lift} sm:grid-cols-[220px_minmax(0,1fr)]`}>
      ...
      <div className="min-w-0 p-2 sm:py-4 sm:pr-5">
        ...
        <h2 className={`mt-3 line-clamp-3 text-2xl font-semibold leading-tight tracking-[-0.02em] ${pal.panelText} sm:text-3xl`}>{post.title}</h2>
        <p className={`mt-4 line-clamp-3 text-sm leading-7 ${pal.softMutedText}`}>{getEditableExcerpt(post, 180)}</p>
        <span className={`mt-5 inline-flex items-center gap-2 text-sm font-semibold ${pal.accentText}`}>Open article <ArrowRight className="h-4 w-4" /></span>
      </div>
    </Link>
  )
}
